...
namespace recompui {
    ...
    // Safe to call after the renderer hook has already shut the UI down.
    // This explicit fallback prevents RmlUi objects from surviving until
    // process-wide static destruction when the launcher window is closed.
    void shutdown();
