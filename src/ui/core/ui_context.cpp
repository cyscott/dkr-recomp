...
void recompui::init_styling(const std::filesystem::path& rcss_file) {
    std::string style{};
    {
        ...
        if (!style_stream) {
            fprintf(stderr, "Failed to open UI stylesheet: %s\n", rcss_file.string().c_str());
            context_state.style_sheet = {};
            return;
        }

        const std::streampos style_size = style_stream.tellg();
        if (style_size <= 0) {
            fprintf(stderr, "UI stylesheet is empty or unreadable: %s\n", rcss_file.string().c_str());
            context_state.style_sheet = {};
            return;
        }

        style.resize(static_cast<std::size_t>(style_size));
        style_stream.seekg(0, std::ios::beg);
        if (!style_stream.read(style.data(), static_cast<std::streamsize>(style.size()))) {
            fprintf(stderr, "Failed to read UI stylesheet: %s\n", rcss_file.string().c_str());
            context_state.style_sheet = {};
            return;
        }
    }
